...
export class EmprestimoService{
    ...
    async filtrarProdutoByDataEmprestimo(emprestimoData: any): Promise<EmprestimoEntity> {
        const dataEmprestimo: number = emprestimoData;

        const emprestimo =  await this.emprestimoRepository.filterEmprestimoById(dataEmprestimo);
        console.log("Service - Filtrar", emprestimo);
        return emprestimo;
    }

    async listarTodosEmprestimo(): Promise<EmprestimoEntity[]> {
        const emprestimo =  await this.emprestimoRepository.filterAllEmprestimo();
