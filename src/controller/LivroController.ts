...
export class LivroController extends Controller {
    ...
    @Get()
    async filtrarLivroPorTitulo(
        @Query() titulo: string,
        @Res() notFound: TsoaResponse<400, BasicResponseDto>,
        @Res() success: TsoaResponse<200, BasicResponseDto>
    ): Promise<void> {
        try {
            const livro: LivroEntity[] = await this.livroService.filtrarLivroByTitulo(titulo);
            return success(200, new BasicResponseDto("Titulo encontrado!", titulo));
        } catch (error: any) {
            return notFound(400, new BasicResponseDto(error.message, undefined));
        }
    }
    ...
    async listarTodosLivros(
        ...
        try {
            const livro: LivroEntity[] = await this.livroService.listarTodosLivros();
